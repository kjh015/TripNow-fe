...
import { getDashboardData } from '../../../api/log/monitoringApi';
import { getFormatKeys } from '../../../api/log/filterApi';
...
const TopFormatDashboard = () => {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const { data } = await getFormatKeys(1);
                const options = data.map(format => ({ label: format, value: format }));
    ...
    const handleFetchData = async () => {
        ...
        setIsLoading(true);
        try {
            const { data } = await getDashboardData({ data: selectedType });
